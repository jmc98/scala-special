...
import FloatOps._
  ...
  test("'insert' should work correctly on a leaf with center (1,1) and size 2") {
    val body1 = new Body(2f, 0.2f, 0.3f, 0.1f, 0.1f)
    val body2 = new Body(3f, 1.4f, 0.4f, 0.1f, 0.1f)
    val leaf = Leaf(1, 1, 2, Seq(body1))

    val quad = leaf.insert(body2)
    assert(quad == Fork(
      nw = Leaf(0.5f, 0.5f, 1, Seq(body1)),
      ne = Leaf(1.5f, 0.5f, 1, Seq(body2)),
      sw = Empty(0.5f, 1.5f, 1),
      se = Empty(1.5f, 1.5f, 1)
    ))
  }
}
