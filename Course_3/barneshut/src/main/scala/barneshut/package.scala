...
package object barneshut {
  ...
  extends Quad {
    override val mass: Float = bodies.map(_.mass).sum
    override val massX: Float = bodies.map(b => b.mass * b.x).sum / mass
    override val massY: Float = bodies.map(b => b.mass * b.y).sum / mass
    ...
    def insert(b: Body): Quad = {
      ...
      if (size <= minimumSize) Leaf(centerX, centerY, size, newBodies)
        ...
        newBodies.foldLeft(emptyFork) {
          (quad, body) => quad.insert(body)
        }
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val xRel = math.max(math.min(b.x, boundaries.maxX), boundaries.minX) - boundaries.minX
      val yRel = math.max(math.min(b.y, boundaries.maxY), boundaries.minY) - boundaries.minY
      apply(x = (xRel / sectorSize).toInt, y = (yRel / sectorSize).toInt) += b
      this
    }
